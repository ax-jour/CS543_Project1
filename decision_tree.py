...
def get_mode(rdd):
    return sorted(rdd.map(lambda x: (x[8], 1)).groupByKey().mapValues(len).collect(), key=lambda x: x[1])[-1][0]
...
def build_tree(rdd, root: Node, max_level: int):
    ...
    while queue:
        ...
        for _ in range(size):
            ...
            # cur_rdd = rdd.filter(lambda x: x[-1] == cur.id)
            cnt = cur_rdd.count()
            ...
            if cnt <= 5000:
                ...
                for value in catagories[split_attr]:
                    ...
                    new_rdd = cur_rdd.filter(lambda x: x[-1] == cur.id and x[split_attr] == value).map(
                        lambda x: x[:9] + [new_id])
...
if __name__ == '__main__':
    ...
    data_path = './output/processed_int/part-00000-58cb0718-851a-45a6-bc50-3346e8bdcbae-c000.csv'
    origin_data = sc.textFile(data_path).map(lambda line: line.split(',')).map(
        lambda x: [eval(item) for item in x] + ['root'])
    print(origin_data.take(5))
